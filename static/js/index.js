...
function initCodeMirrorAndBindings() {
    ...
    window.cm = CodeMirror.fromTextArea(tikzCode, {
        mode: 'stex',
        theme: 'material',
        lineNumbers: true,
        lineWrapping: true,
    ...
    try {
        window.cm.setValue(JSON.parse(document.getElementById('initial-tikz')?.textContent || '""'));
    } catch(e) {
        window.cm.setValue('');
    }
    ...
    if (!window.appState.loggedIn) {
        window.cm.on('mousedown', function() {
            showLoginModal();
        });
    }
    ...
    window.cm.on('change', function() {
        ...
        inputPreviewTimer = setTimeout(() => {
            updateInputPreview(window.cm.getValue());
    ...
    const initialCode = window.cm.getValue();
...
async function submitTikzCodeAjax(event) {
    ...
    const tikzCode = cleanControlChars(window.cm.getValue());
    if (!tikzCode.trim()) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 13) Action buttons đã được initialize trong file_card.js

    // 14) Touch events đã được xử lý trong initializeTouchBtnEvents cho tất cả users

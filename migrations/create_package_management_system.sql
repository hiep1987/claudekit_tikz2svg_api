-- =====================================================
-- PACKAGE MANAGEMENT SYSTEM DATABASE MIGRATION
-- =====================================================
-- Version: 1.0
-- Date: Phase 1 Implementation  
-- Purpose: Create package management tables and indexes
...
CREATE TABLE IF NOT EXISTS package_changelog (
    ...
    change_reason TEXT,
    request_id INT, -- Link to originating request
    ...
    FOREIGN KEY (package_id) REFERENCES supported_packages(id) ON DELETE CASCADE,
    FOREIGN KEY (request_id) REFERENCES package_requests(id) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- =====================================================
-- 4. PACKAGE USAGE STATISTICS TABLE (Optional)
-- =====================================================

CREATE TABLE IF NOT EXISTS package_usage_stats (
    id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    INDEX idx_usage_package (package_id, usage_date DESC),
    INDEX idx_usage_date (usage_date DESC),
    INDEX idx_usage_user (user_id, usage_date DESC),
    INDEX idx_usage_stats (package_name, compilation_count DESC),
...
-- =====================================================
-- 6. CREATE ADMIN USER PERMISSIONS (Optional)
-- =====================================================

-- Add admin permissions table if needed
...
LEFT JOIN package_requests pr ON sp.package_name = pr.package_name AND pr.status = 'implemented'
WHERE sp.status = 'active'
GROUP BY sp.id, sp.package_name, sp.package_type, sp.description, sp.usage_count, sp.status
ORDER BY sp.usage_count DESC, request_count DESC;

-- Pending requests view for admin
CREATE OR REPLACE VIEW pending_requests_admin AS
SELECT 
    pr.*,
    sp.id as existing_package_id,
    CASE WHEN sp.id IS NOT NULL THEN 'duplicate' ELSE 'new' END as request_type
FROM package_requests pr
LEFT JOIN supported_packages sp ON pr.package_name = sp.package_name AND pr.package_type = sp.package_type
...
ANALYZE TABLE supported_packages;
ANALYZE TABLE package_requests;
ANALYZE TABLE package_changelog;
ANALYZE TABLE package_usage_stats;
...
INSERT INTO package_changelog (
    ...
    JSON_OBJECT(
        ...
        'packages_imported', (SELECT COUNT(*) FROM supported_packages),
        'tables_created', 4
    ),
    'system',
    'Package Management System database migration completed'
);

SELECT 
    'Package Management System Migration Complete!' as message,
    (SELECT COUNT(*) FROM supported_packages) as packages_imported,
    (SELECT COUNT(*) FROM supported_packages WHERE package_type = 'latex_package') as latex_packages,
    (SELECT COUNT(*) FROM supported_packages WHERE package_type = 'tikz_library') as tikz_libraries, 
    (SELECT COUNT(*) FROM supported_packages WHERE package_type = 'pgfplots_library') as pgfplots_libraries;

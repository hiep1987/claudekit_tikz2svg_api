...
def api_like_counts():
    ...
    if not isinstance(svg_ids, list):
    ...
    try:
        ...
        if svg_ids:
            ...
            cursor.execute(
                f"SELECT svg_image_id, COUNT(*) as count FROM svg_like WHERE svg_image_id IN ({format_strings}) GROUP BY svg_image_id",
                tuple(svg_ids)
            )
            for row in cursor.fetchall():
                like_counts[str(row[0])] = row[1]
            # Đảm bảo trả về 0 cho các id không có like
            for svg_id in svg_ids:
                if str(svg_id) not in like_counts:
                    like_counts[str(svg_id)] = 0
        cursor.close()
        conn.close()
    except Exception as e:

...
@app.route("/api/followed_posts", methods=["GET"])
def followed_posts():
    """
    API trả về 10 bài đăng SVG gần đây nhất từ những người mà user hiện tại đã theo dõi
    """
    print(f"🔍 DEBUG Followed Posts Request STARTED", flush=True)
    ...
    try:
        ...
        # Query bài đăng từ người đã follow
        query = """
            SELECT 
                si.id, 
                si.filename, 
            ...
            JOIN user_follow uf ON uf.followee_id = si.user_id
            LEFT JOIN svg_like sl ON si.id = sl.svg_image_id
            LEFT JOIN svg_like user_like ON si.id = user_like.svg_image_id AND user_like.user_id = %s
            WHERE uf.follower_id = %s
            GROUP BY si.id, si.filename, si.created_at, si.tikz_code, u.username, u.id, user_like.id
        ...
        print(f"  - ✅ Found {len(posts)} followed posts", flush=True)
        return jsonify(posts)
        
    except Exception as e:
        print(f"[ERROR] followed_posts: {e}", flush=True)
        import traceback
        print(f"[ERROR] Traceback: {traceback.format_exc()}", flush=True)
        return jsonify({"error": "Lỗi khi lấy bài đăng"}), 500
    finally:
        try:
            if cursor:
                cursor.close()

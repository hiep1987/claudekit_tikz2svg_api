...
def save_svg():
    ...
    try:
        ...
        cursor.execute(
            "INSERT INTO svg_image (filename, keywords, user_id) VALUES (%s, %s, %s)",
            (svg_filename, keywords, user_id)
        )
        conn.commit()
        cursor.close()
        conn.close()
...
def view_svg(filename):
    ...
    # ✅ NEW: Lấy "Người tạo: username" từ DB
    display_name = filename
    if '_' in filename:
        possible_google_id = filename.split('_')[0]
        try:
            conn = mysql.connector.connect(
                host=os.environ.get('DB_HOST', 'localhost'),
                user=os.environ.get('DB_USER', 'hiep1987'),
            ...
            cursor = conn.cursor()
            cursor.execute("SELECT username FROM user WHERE google_id = %s", (possible_google_id,))
            result = cursor.fetchone()
            if result and result[0]:
                display_name = f"Người tạo: {result[0]}"
            cursor.close()

...
def after_auth():
    # Không cần kiểm tra google.authorized ở đây nữa, Flask-Dance đã xử lý
    # Nếu không ủy quyền, nó sẽ tự động redirect về google.login
    try:
        ...
        if resp.ok:
            ...
            flash(f"Chào mừng, {info['email']}!", category="success") 
            
            # >>> ĐÂY LÀ THAY ĐỔI LỚN NHẤT <<<
            # BỎ return render_template('login_success_redirect.html')
            # Để Flask-Dance tự xử lý chuyển hướng đến 'index' (như đã cấu hình redirect_to)
            # Không cần return gì ở đây nữa, hoặc return None/'' để báo hiệu xử lý xong.
            # Flask-Dance sẽ intercept response này.
            return "" # Trả về chuỗi rỗng hoặc None, Flask-Dance sẽ lấy và redirect

        else:
            del google.token
            flash("Đăng nhập thất bại.", category="error")
            # Nếu có lỗi, bạn vẫn có thể redirect thủ công
            return redirect(url_for("google.login"))
    except Exception as e:
        del google.token
        flash(f"Lỗi đăng nhập: {e}", category="error")
        # Nếu có lỗi, bạn vẫn có thể redirect thủ công
        return redirect(url_for("google.login"))
...
def load_user_info_if_missing():
    # Bỏ qua logic này cho các route liên quan đến quá trình OAuth ban đầu
    # hoặc các file tĩnh để tránh xung đột và vòng lặp.
    if request.path.startswith('/login/google/authorized') or \
       ...
       request.path.startswith('/logout'): # Có thể thêm /logout nếu bạn có
        return

    # Nếu người dùng đã được ủy quyền (đã đăng nhập Google qua OAuth)
    # NHƯNG 'user_email' lại không có trong session (ví dụ: do session cookie chưa được gửi đúng ở lần đầu,
    # hoặc session đã hết hạn/bị xóa sau đó)
    if google.authorized and "user_email" not in session:
        ...
        try:
            ...
            if resp.ok:
                ...
                # Nếu không thể re-fetch (ví dụ: token hết hạn), xóa token và để ứng dụng xử lý
                del google.token
                print("DEBUG: Failed to re-fetch userinfo. Google token might be invalid, clearing.", flush=True)
                # KHÔNG REDIRECT Ở ĐÂY. Để các route riêng lẻ kiểm tra google.authorized và tự redirect nếu cần.
        except Exception as e:
            del google.token
...
def inject_user_info():
    ...
    user_email = session.get('user_email')
    print(f"DEBUG Context processor - user_email: {user_email}")
    if user_email:
        user = get_user_by_email(user_email)
        print(f"DEBUG Context processor - user data: {user}")
        if user:
            avatar_url = url_for('static', filename=f'avatars/{user["avatar"]}') if user.get("avatar") else None
            print(f"DEBUG Context processor - avatar_url: {avatar_url}")
            return {
                'user_email': user_email,
